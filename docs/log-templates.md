...
`start` / `end` 是边界规则，只支持 `source`、`match`、`values`、`reprocess`、`message` 和 `message_tone`。`tone` 不能写在边界规则中；block 的默认颜色写在 `[blocks.<name>]`，边界附带 `message` 时才用 `message_tone` 控制该消息颜色。
...
每个新 retry 创建自己的日志缓冲区时都会重新读取 TOML，因此保存模板后不需要重启后端；已经开始的 retry 保持创建时的模板，避免运行中途改变 block 状态机。

运行时使用容错加载：未知字段会被忽略，坏 rule、坏 `start` / `end` 或坏 block 只隔离自身，其余有效翻译继续工作。每个具体错误以 `WARNING` 写入 `data/debug/framework/framework.log`。如果 TOML 语法、文件读取或版本本身无法解析，则使用进程内最近一次有效模板；进程启动后还没有有效模板时退回原始行显示。

每个受影响的 retry 只向可见日志加入一条稳定 framework event，SSE 仍正常连接并继续传输后续日志。模板和其他可见日志配置器在任何异常下都不得阻止外部命令执行。
