...
def load_blocks(
    ...
    for name, raw in value.items():
        ...
        reject_unknown_keys(path, raw, allowed, location)
        rules = load_rules(path, raw.get("rules"), fields, lookups, location=location)
        start = load_boundaries(path, raw.get("start"), location=location, boundary="start")
        end = load_boundaries(path, raw.get("end"), location=location, boundary="end")
        translations = string_mapping(path, raw.get("translations"), f"{location}.translations")
        ...
        captures = tuple(
            dict.fromkeys(capture for rule in (*start, *end) for capture in rule.pattern.captures)
        )
...
def load_rules(
    ...
    *,
    ...
    allowed = {
        "match", "action", "text", "tone", "indent", "styles", "segments", "lookups",
        "replacements", "values", "fold_group", "fold_role",
    }
    for index, raw in enumerate(value):
        rule_location = f"{location}.rules[{index}]"
        if not isinstance(raw, dict):
            raise TemplateValidationError(path, rule_location, "must be a table")
        reject_unknown_keys(path, raw, allowed, rule_location)
...
def load_boundaries(
    ...
    *,
    ...
    for index, raw in enumerate(value):
        ...
        reject_unknown_keys(path, raw, {"source", "match", "values", "reprocess", "message", "message_tone"}, item_location)
        raw_values = raw.get("values")
        if raw_values is not None and not isinstance(raw_values, dict):
