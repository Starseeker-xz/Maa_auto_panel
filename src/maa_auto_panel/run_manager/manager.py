...
from maa_auto_panel.run_manager.context import RetryContext, RunCallbackAPI
from maa_auto_panel.run_manager.logs import RunLogProfile
from maa_auto_panel.run_manager.state import LiveRetry, LiveRun, RunTimeouts, now_text
...
class GenericRunManager:
    ...
    def _execute_loop(self, state: LiveRun, plan: RunStartPlan, lease: RunLease) -> None:
        ...
        next_payload = dict(plan.initial_retry_payload)

        try:
            for retry_index in range(1, state.max_retries + 1):
                ...
                start_decision = self._run_start_hooks_if_needed(context, plan)
                if start_decision is not None:
                    ...
                    final_completion = self._run_finish_callback(context.api, plan, final_completion)
                    self._run_scripts(state, plan, retry, context, "after_run")
                ...
                if retry_index == 1:
                    try:
                        self.coordinator.acquire(
                            ...
                            on_wait=lambda blockers: self._report_resource_wait(context, blockers),
                        ...
                        context.add_event(self._resource_conflict_message(exc), tone="danger")
                        ...
                        self._finish_retry_from_decision(state, retry, decision)
                        break
                    self._report_resource_acquired(context)
                    if state.stop_requested:
                        decision = RetryDecision("stopped", None, continue_retry=False, run_status="stopped")
                        self._finish_retry_from_decision(state, retry, decision)
                        final_completion = RunCompletion(status="stopped")
                ...
                command = self._command_for(plan, context, next_command)
                if command is None:
                    decision = RetryDecision("failed", None, continue_retry=False)
                    context.add_event(plan.text.start_failed.format(error="no command"), tone="danger")
                else:
                    decision = self._run_retry_command(state, plan, retry, context, command)

                decision = self._after_retry(state, plan, retry, context, decision)
                if state.stop_requested:
                ...
                should_continue = bool(decision.continue_retry and retry_index < state.max_retries and not state.stop_requested)
                if should_continue and plan.text.retry_next:
                    context.add_event(plan.text.retry_next, tone="warning")
                elif decision.continue_retry and retry_index >= state.max_retries and plan.text.retry_limit_reached:
                    context.add_event(plan.text.retry_limit_reached, tone="danger")

                if should_continue:
                    next_command = decision.next_command or next_command
                    next_payload = dict(decision.next_retry_payload or next_payload)
                else:
    ...
    def _report_resource_wait(self, context: RetryContext, blockers: list[RunLease]) -> None:
        ...
        context.add_event(f"等待运行资源释放: {blocker_text}", tone="warning")

    def _report_resource_acquired(self, context: RetryContext) -> None:
        state = self._state_for(context)
        ...
        with self._lock:
            ...
            self._notify_locked()
        self.store.update_run_metadata(state.id, {"resource_wait": resource_wait})
        context.add_event("运行资源已取得，继续执行。", tone="info")
    ...
    def _run_start_hooks_if_needed(self, context: RetryContext, plan: RunStartPlan) -> RetryDecision | None:
        state = self._state_for(context)
        retry = self._retry_for(context)
        if context.retry_index == 1:
            decision = plan.callbacks.on_start(context) if plan.callbacks.on_start is not None else None
            if decision is not None:
                return decision
            if state.stop_requested:
                return RetryDecision("stopped", None, run_status="stopped")
            self._run_scripts(state, plan, retry, context, "before_run")
            if state.stop_requested:
                return RetryDecision("stopped", None, run_status="stopped")
            if plan.text.start:
                context.add_event(plan.text.start.format(title=plan.title, retry_index=context.retry_index), tone="info")
            return None

        if state.stop_requested:
            return RetryDecision("stopped", None, run_status="stopped")
        if plan.callbacks.before_retry is not None and context.previous_decision is not None:
            plan.callbacks.before_retry(context, context.previous_decision)
        if state.stop_requested:
            return RetryDecision("stopped", None, run_status="stopped")
        self._run_scripts(state, plan, retry, context, "before_retry")
        if state.stop_requested:
            return RetryDecision("stopped", None, run_status="stopped")
        if plan.text.retry_start:
            context.add_event(plan.text.retry_start.format(title=plan.title, retry_index=context.retry_index), tone="warning")
        return None

    def _command_for(self, plan: RunStartPlan, context: RetryContext, previous_command: CommandSpec | None) -> CommandSpec | None:
        if plan.callbacks.before_retry_execution is not None:
            plan.callbacks.before_retry_execution(context)
        if plan.callbacks.build_command is not None:
            return plan.callbacks.build_command(context)
    ...
    def _run_retry_command(
        ...
        try:
            result = self.run_process(
                ...
                on_raw_line=lambda stream, line: self._handle_raw_line(plan, context, stream, line),
                on_timeout=lambda level, elapsed: self._append_timeout_event(context, level, elapsed),
            )
        except Exception as exc:
            context.add_event(plan.text.start_failed.format(error=exc), tone="danger")
            return RetryDecision("failed", None)

        decision = plan.callbacks.evaluate_retry(context, result) if plan.callbacks.evaluate_retry is not None else None
        if decision is None:
        ...
        if result.stopped or state.stop_requested:
            ...
            decision.retry_status = "stopped"
            decision.run_status = "stopped"
        self._append_default_result_event(context, plan, decision)
        return decision
    ...
    def _after_retry(
        ...
        if plan.callbacks.after_retry is not None:
            updated = plan.callbacks.after_retry(context, _result_from_decision(decision), decision)
            if updated is not None:
                decision = updated
        self._run_scripts(state, plan, retry, context, "after_retry")
        return decision
    ...
    def _run_scripts(
        ...
        for script in scripts:
            try:
                command = script.command(context) if callable(script.command) else script.command
            except FileNotFoundError:
                context.add_event(f"脚本不存在: {script.label}", tone="warning")
                continue
            except Exception as exc:
                context.add_event(f"脚本启动失败: {exc}", tone="danger")
                continue
            if command is None:
                continue
            context.add_event(f"运行脚本({hook}): {script.label}", tone="info")
            ...
            try:
                result = self.run_process(
                    ...
                    on_timeout=lambda level, elapsed: self._append_script_timeout_event(context, level, elapsed),
                )
            except Exception as exc:
                context.add_event(f"脚本启动失败: {exc}", tone="danger")
                ...
                continue
            if result.timed_out:
                context.add_event("脚本运行超时，已终止。", tone="danger")
            if result.return_code != 0:
                context.add_event(f"脚本退出码: {result.return_code}", tone="warning")
    ...
    def _finish_retry(
        ...
        *,
        ...
        try:
            ...
            # Retry details are observational history. A failed detail write must
            # not stop the domain retry loop or weaken durable run finalization.
            logger.exception(
                "retry history persistence failed; continuing run_id=%s retry_id=%s",
                state.id,
                retry.id,
            )
    ...
    def _persist_completion(self, state: LiveRun, completion: RunCompletion) -> None:
        for persist_try in range(1, FINALIZATION_PERSIST_ATTEMPTS + 1):
            try:
                ...
                if persist_try >= FINALIZATION_PERSIST_ATTEMPTS:
                    raise
                logger.exception(
                    "run finalization persistence failed; retrying run_id=%s kind=%s try=%s/%s",
                    state.id,
                    state.kind,
                    persist_try,
    ...
    def _handle_raw_line(self, plan: RunStartPlan, context: RetryContext, stream: str, line: str) -> None:
        if plan.callbacks.on_raw_line is not None:
            plan.callbacks.on_raw_line(context, stream, line)
    ...
    def _append_default_result_event(self, context: RetryContext, plan: RunStartPlan, decision: RetryDecision) -> None:
        if decision.retry_status == "succeeded" and plan.text.completed:
            context.add_event(plan.text.completed, tone="success")
            return
        if plan.text.exit_code:
            tone = "warning" if decision.retry_status == "stopped" else "danger"
            if decision.return_code == 0:
                tone = "info"
            context.add_event(plan.text.exit_code.format(return_code=decision.return_code), tone=tone)
    ...
    def _append_timeout_event(self, context: RetryContext, level: str, elapsed: float) -> None:
        message = _timeout_message(level, elapsed, process_name=self._plan_for(self._state_for(context)).text.process_name)
        tone = "warning" if level.endswith("warning") else "danger"
        context.add_event(message, tone=tone)

    def _append_script_timeout_event(self, context: RetryContext, level: str, elapsed: float) -> None:
        if level == "runtime_kill":
            context.add_event(f"脚本运行超过 {elapsed:.0f}s，正在终止。", tone="danger")
    ...
    def _state_for(self, context: RetryContext) -> LiveRun:
        state = self.get(context.run_id)
        if state is None:
            raise KeyError(context.run_id)
        return state

    def _retry_for(self, context: RetryContext) -> LiveRetry:
        state = self._state_for(context)
        for retry in state.retries:
            if retry.id == context.retry_id:
                return retry
        raise KeyError(context.retry_id)
