...
Usage:
    python -m maa_auto_panel.tools.game update-game [options]
    python -m maa_auto_panel.tools.game get-download-link [--game-id ID]
"""
...
from maa_auto_panel.paths import CACHE_DIR_ENV
from maa_auto_panel.settings import DEFAULT_DEVICE_SERIAL, DEFAULT_GAME_ID, DEFAULT_TARGET_PACKAGE
from maa_auto_panel.tools.game.update import get_android_download_link, update_game


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="python -m maa_auto_panel.tools.game")
    ...
    default_cache_root = Path(os.environ.get(CACHE_DIR_ENV, "cache"))
    update_parser.add_argument(
        "--download-dir",
        default=str(default_cache_root / "downloads"),
        help=f"APK cache directory (default: {CACHE_DIR_ENV}/downloads or cache/downloads)",
    )
