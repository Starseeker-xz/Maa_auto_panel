...
from maa_auto_panel.android.adb import ADBDevice
from maa_auto_panel.settings import (
    BILIGAME_API_URLS,
    DEFAULT_GAME_ID,
...
class PackageManager:
    ...
    def update_package_status(
        ...
        *,
        ...
        if path is not None:
            pkg["path"] = self._stored_path(path)
        if url is not None:
            pkg["url"] = url
        ...
        if patch_path is not None:
            pkg["patch_path"] = self._stored_path(patch_path)
        self._save_manifest()
        self._cleanup_cache()
    ...
    def get_verified_package_path(self, version: int | str) -> Path | None:
        ...
        if not pkg or pkg.get("status") != "verified":
            return None
        path = self._resolved_path(pkg.get("path"))
    ...
    def _remove_cached_file(self, file_path: str | None, label: str) -> None:
        if not file_path:
            return
        path = self._resolved_path(file_path)
        if not path.exists():
            return
        try:
            path.unlink()
...
def ensure_latest_package(manager: PackageManager, game_info: GameInfo, *, force_full: bool = False) -> Path | None:
    ...
    if pkg_info and not force_full:
        path = manager._resolved_path(pkg_info.get("path"))
