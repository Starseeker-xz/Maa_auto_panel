...
from maa_auto_panel.config.tasks import strip_framework_task_metadata


FRAMEWORK_METADATA_SCHEMA: dict[str, Any] = {
...
class ConfigSchemaValidator:
    """Validates maa-cli configs against JSON schemas and framework metadata schema."""
    def __init__(self, runtime: MaaRuntime) -> None:
        self.runtime = runtime

    @cached_property
    def maa_task_schema(self) -> dict[str, Any]:
        schema_path = self.runtime.maa_schema_dir / "task.schema.json"
        return json.loads(schema_path.read_text(encoding="utf-8"))

    @cached_property
    def maa_profile_schema(self) -> dict[str, Any]:
        schema_path = self.runtime.maa_schema_dir / "asst.schema.json"
        return json.loads(schema_path.read_text(encoding="utf-8"))

    @cached_property
    def maa_cli_schema(self) -> dict[str, Any]:
        schema_path = self.runtime.maa_schema_dir / "cli.schema.json"
        return json.loads(schema_path.read_text(encoding="utf-8"))
    ...
    def metadata_validator(self) -> Draft7Validator:
        return Draft7Validator(FRAMEWORK_METADATA_SCHEMA)
                ...
                if not isinstance(metadata, dict):
                    errors.append(
                        ConfigValidationError(
                            path=f"tasks[{index}].framework",
                            message="framework metadata must be an object",
                            source="framework",
                        )
                    )
                    continue
