...
RUNTIME_PLACEHOLDER_PREFIX = "__linux_maa_runtime__:"
...
def prepare_framework_task_config(data: dict[str, object], runtime: object, messages: list[str] | None = None) -> dict[str, object]:
    ...
    for index, task in enumerate(tasks, start=1):
        if not isinstance(task, dict):
            continue
        raw_metadata = task.get("linux_maa")
...
def task_item_to_config_task(item: dict[str, Any]) -> dict[str, Any]:
    ...
    metadata = deepcopy(item.get("linux_maa")) if isinstance(item.get("linux_maa"), dict) else {}
...
def _normalize_fight_stage_value(value: object) -> str:
    from linux_maa.maa.stages import normalize_stage_plan_value
