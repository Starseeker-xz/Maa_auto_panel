...
class Diagnostics:
    ...
    def capture_maacore_log(self, run_id: str, start_offset: int) -> str | None:
        ...
        return relative_path(target, self.runtime.repo_root)
...
def configure_framework_logging(runtime: MaaRuntime, retention: LogRetentionPolicy | None = None) -> logging.Logger:
    ...
    for handler in list(logger.handlers):
        if getattr(handler, "_linux_maa_framework_handler", False):
    ...
    handler._linux_maa_framework_handler = True  # type: ignore[attr-defined]
    logger.addHandler(handler)
    logger.debug("framework logging configured path=%s", relative_path(log_file, runtime.repo_root))
    return logger
