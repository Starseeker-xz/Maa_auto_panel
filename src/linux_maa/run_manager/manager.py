...
from linux_maa.process import RawLineCallback, StreamingProcessResult, TimeoutCallback, run_streaming_process
from linux_maa.run_manager.command import CommandSpec
from linux_maa.run_manager.coordinator import RunCoordinator, RunLease
from linux_maa.run_manager.logs import RunLogProfile
from linux_maa.run_manager.state import LiveRetry, LiveRun, RunKind, RunTimeouts, now_text
from linux_maa.run_manager.store import RunStateStore
from linux_maa.run_resources import RUN_PRIORITY_NORMAL, RUN_PRIORITY_VALUES, RunResource
...
class GenericRunManager:
    ...
    def start(self, plan: RunStartPlan, *, run_id: str | None = None) -> LiveRun:
        ...
        thread = threading.Thread(target=self._execute_loop, args=(state, plan), daemon=True)
        state.thread = thread
        try:
            thread.start()
        except Exception:
