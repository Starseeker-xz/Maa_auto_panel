...
- `runtime/maa` 位于独立 runtime volume 中，与框架 data 和应用镜像版本解耦。
...
容器安装 `adb` 并运行自己的 ADB server：

```text
panel container -> bridge/LAN -> 192.168.5.151:5555
```

不连接宿主 ADB server，也不需要：
...
路径管理重构后，框架拥有的数据位于 `/app/data`，集成安装位于 `/app/runtime`；可丢弃 download cache 与 ADB credential state 使用独立边界：

| 容器路径 | 内容 | 第一版建议 | 备份级别 |
|---|---|---|---|
| `/app/data` | config/state/history/debug | bind mount | 框架主持久卷 |
| `/app/runtime` | maa-cli/MaaCore/resource/XDG state | bind mount | 集成运行时卷；可独立更新或重建 |
...
RUN useradd --create-home --no-log-init --uid 10001 panel \
 && mkdir -p data runtime cache/downloads /home/panel/.android \
...
services:
  panel:
    ...
    volumes:
      ...
      - /srv/maa-auto-panel/runtime:/app/runtime
...
bind mount 会遮蔽镜像中目标目录原有内容，因此不要把 baseline runtime 先放进 `/app/runtime` 再指望首次挂载后仍可见。`init-runtime` 应从镜像内只读 seed（例如 `/opt/maa-runtime-seed`）复制，或按固定 URL + checksum 下载到空卷。宿主 bind 目录必须在启动前创建并授予 UID/GID 10001；非 root entrypoint 只能检查权限，不能可靠地替宿主 `chown`。
...
- `/app/runtime/maa/bin/maa` 存在且可执行。
