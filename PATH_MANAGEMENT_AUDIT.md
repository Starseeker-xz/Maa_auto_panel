...
建议把框架拥有或运行时依赖的持久数据统一收敛到一个可配置的 `data_root`。开发环境默认使用仓库内 `data/`，容器环境把它挂载到 `/app/data`（或把 `data_root` 指向 `/data`）。目录内部继续按职责和保留策略分区，不能把 config、state、history、debug 混放。

`data_root` 不是“所有需要持久化的文件”的总称。可重新下载、删除后不影响框架自身状态完整性的 APK/patch 属于 cache；ADB 客户端密钥属于外部连接凭据。两者都应与框架 data 分开。
...
外层 `data/` 表示 Maa Auto Panel 的持久化数据根；`runtime/maa/data` 是 maa-cli 的 XDG data home。名字重复但语义明确，第一轮不值得为了外观改变上游运行时布局。

本机已按该布局完成迁移。框架 data 使用一个主边界；大体积 download cache 和 ADB credential state 各自使用独立、可选择不同保留策略的 volume。应用代码与静态资产可保持只读。

## 2. 当前路径全景

当前所有主要路径都由 `MaaRuntime(repo_root)` 间接或直接绑定到仓库根：
...
| `runtime/maa` | maa-cli、MaaCore、resource、XDG cache/state、生成配置 | 可更新安装与运行数据 | 约 1.3 GiB | `data/runtime/maa` |
...
当前至少有六个仓库内可写根，另有 `$HOME/.android`。应按所有权和可恢复性收敛成三类边界，而不是强塞进一个父目录：

1. `data_root`：框架拥有或运行时依赖的数据，包括 config/state/history/debug/runtime。
2. `cache_root`：删除后可重新生成或下载的数据；当前主要是 downloads。
3. `adb-state`：ADB 客户端授权密钥的独立容器 volume，不扩展成通用持久 HOME。
...
`runtime/maa/generated-configs` 和 `run-logs` 是可清理数据，却与可在线更新的 binary/resource 同处一棵树。第一版可保持兼容；后续若需要不同 volume/retention，再把它们映射到 `data/cache/generated-configs/maa` 和 `data/debug/legacy-run-logs/maa`，不应阻塞本次收敛。
...
- 开发默认生成职责明确的 `data/` 与 `cache/`；不再生成散落的顶层 config/state/history/debug/runtime/downloads。
