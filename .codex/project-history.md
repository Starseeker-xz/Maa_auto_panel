...
- Confirmed (`2026-07-02_2245-tools-page`): Tool logs now intentionally use the same visible-log path as maa-cli runs. Raw stdout/stderr are written to diagnostics, then `ToolRunManager` passes chunks through `MaaCliLogTranslator.translate(..., source=stream)`; framework events use `MaaCliLogTranslator.add_event()`, and process completion flushes the translator. This keeps translation, source-aware state, `output`, `task_results`, and `log_entries` behavior aligned with manual maa-cli logs.
...
- Confirmed (`2026-07-02_2245-tools-page`): `run_streaming_process()` now preserves raw carriage returns using `TextIOWrapper(..., newline="")`, and `MaaCliLogTranslator` collapses terminal redraw/progress output (`\r` and backspace/delete edits) into a single mutable line entry. This fixes `tqdm` download progress flooding without adding downloader-specific throttling.
...
- Confirmed (`2026-07-02_1933-config-sync-ui-schema`): Latest scheduled run summary split diagnosis: run `2b1fbfec1038` had `Summary` and summary body on stdout, while stderr emitted timestamped `AllTasksCompleted`. The current `MaaCliLogTranslator` clears `_current_summary` when any timestamped line arrives; if merged live stream ordering delivers stderr `AllTasksCompleted` between stdout `Summary` and the later stdout summary body, the summary block becomes empty and the body lines render as standalone line cards. This is a confirmed root cause to fix later.
- Confirmed (`2026-07-02_1933-config-sync-ui-schema`): The scheduled/manual live log translator is now source-aware. `run_streaming_process()` reads stdout/stderr separately; manual and scheduled `_append_maa_log()` now pass the stream into `MaaCliLogTranslator.translate(text, source=stream)`. The translator keeps partial lines, active task, active summary, and active git-output blocks per source, so stderr timestamped lifecycle lines no longer close stdout summary blocks. SSE reconnect recovery now always resends full `log_entries` when the client version is behind, avoiding missed updates to older structured log entries. Normal online SSE still sends first-difference array patches.
