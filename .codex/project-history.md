...
- Confirmed (`2026-07-03_1926-project-review`): Deleted 4 dead compatibility re-export modules (`adb.py`, `constants.py`, `game_update.py`, `maa_runner.py`), entire `maa/logs/` compat directory, and compatibility aliases (`MaaLogMessage`, `MaaSummaryLogRecord`, `MaaTaskLogRecord`, `MaaCliLogTranslator`) — all with zero imports anywhere. Frontend `logs.ts` retained `STATUS_LABELS` (actively used) and removed dead `translateLogLine`. 45 tests pass, frontend builds clean.
...
- Superseded (`2026-07-03_1926-project-review`): The old `src/linux_maa/maa/logs/` compatibility export layer was deleted after confirming zero imports from `linux_maa.maa.logs` anywhere in src or tests.
...
- Superseded (`2026-07-03_1926-project-review`): `MaaCliLogTranslator` was a compatibility alias for `RunLogTranslator` in old `maa/logs/__init__.py`; deleted along with the entire `maa/logs/` directory.
