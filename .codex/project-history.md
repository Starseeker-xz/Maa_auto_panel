...
- Confirmed (`2026-07-14_1304-investigate-9am-schedule`): Git fetch 的 `From https://github.com/...` 与 ` * branch ... -> FETCH_HEAD` 原样写入 maa-cli stderr，source 默认会呈现 warning。按产品展示选择在 `blocks.fetch.rules` 用两条精确 `tone=info` 规则覆盖；不改变通用 stderr tone 优先级，也不使用会吞掉未知证据的 catch-all。
- Confirmed (`2026-07-14_1304-investigate-9am-schedule`): MaaCore 的 `ProductOfFacility` 不只表示生产产物；`SkillLevel`、`MoodAddition`、`Drone`、`General`、`HR` 是训练室、控制中枢/宿舍、发电站、会客室、办公室的占位类型，且会早于对应 `EnterFacility` 输出。可见 task 日志用五条精确 drop 规则隐藏；PureGold/SyntheticJade/Money/OriginStone 等真实产物继续展示。
